...
type
  TProductosenStock = class(TForm)
    ...
    ZQGrillamovimdepodetallecantidadenviar: TFloatField;
    ...
    procedure ZQGrillaBeforePost(DataSet: TDataSet);
  private
  ...
  public
    ...
    deposito_iddestino:string;
  end;

var
  ProductosenStock: TProductosenStock;
...
procedure TProductosenStock.buscarKeyPress(Sender: TObject; var Key: Char);
begin
    if key=#13 then
      begin
          btnbuscar.Click;
...
procedure TProductosenStock.btnaceptarClick(Sender: TObject);
begin
...
procedure TProductosenStock.Button5Click(Sender: TObject);
var
  producto_campo:string;
begin
    ZQGrilla.Active:=false;
    ZQGrilla.SQL.Text:='select *, 0.00 as movimdepodetallecantidadenviar from productos '+
                       ...
                       'inner join productodeposito productodeposito2 on productos.producto_id=productodeposito2.producto_id and productodeposito2.deposito_id='+deposito_iddestino+
                       ' where 1=1 and productodeposito.deposito_id="'+Princ.dep_id+'" and producto_estado="DISPONIBLE" and producto_tipo="PRODUCTO" and productodeposito.producdepo_stockactual>0 ';
...
//    ZQGrilla.SQL.Text:=ZQGrilla.SQL.Text+'and producto_codigoreferencia '+producto_codigoreferencia;
    ...
    ZQGrilla.SQL.Text:=ZQGrilla.SQL.Text+' group by productos.producto_id order by producto_nombre';

    ZQGrilla.Active:=true;
end;

procedure TProductosenStock.DBGrid1TitleClick(Column: TColumn);
begin
    ZQGrilla.IndexFieldNames:=Column.FieldName;
end;

procedure TProductosenStock.fil_producto_idKeyPress(Sender: TObject;
  var Key: Char);
begin
    if key=#13 then
      Button5.Click;
...
procedure TProductosenStock.FormCreate(Sender: TObject);
begin
    ConfCampoBusqueda1:='producto_id';
    buscar.Text:=princ.ultima_busqueda_productos;
end;

procedure TProductosenStock.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TProductosenStock.FormShow(Sender: TObject);
var
  i: Integer;
  grid_col:integer;
begin
...
procedure TProductosenStock.ZQGrillaBeforePost(DataSet: TDataSet);
begin
    if ZQGrilla.FieldByName('movimdepodetallecantidadenviar').AsFloat>ZQGrilla.FieldByName('producdepo_stockactual').AsFloat then
      ZQGrilla.FieldByName('movimdepodetallecantidadenviar').AsFloat:=ZQGrilla.FieldByName('producdepo_stockactual').AsFloat;
end;
