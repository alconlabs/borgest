unit UnitMovimientosDepositosConfirma;
...
type
  TMovimientosDepositosConfirma = class(TABMbase)
    ...
    ZQmovimdepodetallesmarca_id: TIntegerField;
    ZQmovimdepodetallesseccion_id: TIntegerField;
    ZQmovimdepodetallesrecibido: TFloatField;
...
var
  MovimientosDepositosConfirma: TMovimientosDepositosConfirma;
...
procedure TMovimientosDepositosConfirma.modificar;
begin
    ZQExecSQL.Sql.Clear;
    ...
    ZQExecSQL.ParamByName('movimientodeposito_estadosinc').AsString:='CONFIRMAR';
    ZQExecSQL.ParamByName('movimientodeposito_estado').AsString:='RECIBIDO';
    ...
    while not ZQmovimdepodetalles.Eof do
        begin
            ZQExecSQL.Sql.Clear;
            ZQExecSQL.Sql.Add('update movimdepodetalles set ');
            ...
            ZQExecSQL.Sql.Add('where movimdepodetalle_id=:movimdepodetalle_id ');
            ZQExecSQL.ParamByName('movimdepodetalle_estado').AsString:='RECIBIDO';
            ZQExecSQL.ParamByName('movimdepodetalle_cantidadrecibir').AsString:=ZQmovimdepodetalles.FieldByName('recibido').AsString;
            ZQExecSQL.ParamByName('movimdepodetalle_id').AsString:=ZQmovimdepodetalles.FieldByName('movimdepodetalle_id').AsString;
            ZQExecSQL.ExecSql;

            Princ.actualizarstock(ZQmovimdepodetalles.FieldByName('producto_id').AsString,ZQmovimdepodetalles.FieldByName('recibido').AsFloat,'',false);
...
procedure TMovimientosDepositosConfirma.agregar;
begin
    ...
    ZQExecSQL.ParamByName('movimientodeposito_observaciones').AsString:=movimientodeposito_observaciones.Text;
    ZQExecSQL.ParamByName('movimientodeposito_estadosinc').AsString:='PENDIENTE';
...
function TMovimientosDepositosConfirma.control:boolean;
var
  error:integer;
begin
    error:=0;
...
procedure TMovimientosDepositosConfirma.btnguardarClick(Sender: TObject);
begin
  inherited;
    case abm of
        ABM_AGREGAR:begin
...
procedure TMovimientosDepositosConfirma.btnimprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TMovimientosDepositosConfirma.btnquitarClick(Sender: TObject);
begin
  inherited;
...
procedure TMovimientosDepositosConfirma.Button1Click(Sender: TObject);
begin
  inherited;
...
procedure TMovimientosDepositosConfirma.FormCreate(Sender: TObject);
begin
  inherited;
    deposito_idorigen.llenarcombo;
    deposito_idorigen.Buscar(princ.dep_id);

    deposito_iddestino.Confsql.Text:='select * from depositos order by deposito_nombre';
...
procedure TMovimientosDepositosConfirma.FormShow(Sender: TObject);
begin
  inherited;
    if abm=1 then
      id:='';
    ...
    if ZQSelect.FieldByName('movimientodeposito_estado').AsString='RECIBIDO' then
      btnguardar.Enabled:=false;

end;

procedure TMovimientosDepositosConfirma.ZQSelectAfterOpen(DataSet: TDataSet);
begin
  inherited;
    if abm=ABM_AGREGAR then
      begin
