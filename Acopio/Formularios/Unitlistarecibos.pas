...
type
  TListaRecibos = class(Tlistabase)
...
procedure TListaRecibos.btnanularClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_RECIBO,ABM_ANULAR);
end;

procedure TListaRecibos.btneliminarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_RECIBO,ABM_ELIMINAR);
end;

procedure TListaRecibos.btnfiltrarClick(Sender: TObject);
begin
  inherited;
    ...
    ZQGrilla.SQL.Text:='select * from documentos '+
                       ...
                       ' where tipodocu_nombre="'+TIPODOCU_RECIBO+'" '+Princ.empresa_where;
...
procedure TListaRecibos.btnmodificarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_RECIBO,ABM_MODIFICAR);
end;

procedure TListaRecibos.btnnuevoClick(Sender: TObject);
begin
  inherited;
    Princ.AbrirDocumento('',TIPODOCU_RECIBO,ABM_AGREGAR);
end;
