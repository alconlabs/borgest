unit UnitlistaNotasCreditoVentas;
...
type
  TListaNotasCreditoVenta = class(Tlistabase)
...
var
  ListaNotasCreditoVenta: TListaNotasCreditoVenta;
...
procedure TListaNotasCreditoVenta.btnanularClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_NOTACREDITOVENTA,ABM_ANULAR);
end;

procedure TListaNotasCreditoVenta.btneliminarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_NOTACREDITOVENTA,ABM_ELIMINAR);
end;

procedure TListaNotasCreditoVenta.btnfiltrarClick(Sender: TObject);
begin
  inherited;
    ...
    ZQGrilla.SQL.Text:='select * from documentos '+
                       ...
                       ' where tipodocu_nombre="'+TIPODOCU_NOTACREDITOVENTA+'" '+Princ.empresa_where;
...
procedure TListaNotasCreditoVenta.btnmodificarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_NOTACREDITOVENTA,ABM_MODIFICAR);
end;

procedure TListaNotasCreditoVenta.btnnuevoClick(Sender: TObject);
begin
  inherited;
    Princ.AbrirDocumento('',TIPODOCU_NOTACREDITOVENTA,ABM_AGREGAR);
end;
