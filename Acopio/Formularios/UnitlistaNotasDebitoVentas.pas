unit UnitlistaNotasDebitoVentas;
...
type
  TListaNotasDebitoVenta = class(Tlistabase)
...
var
  ListaNotasDebitoVenta: TListaNotasDebitoVenta;
...
procedure TListaNotasDebitoVenta.btnanularClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_NOTADEBITOVENTA,ABM_ANULAR);
end;

procedure TListaNotasDebitoVenta.btneliminarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_NOTADEBITOVENTA,ABM_ELIMINAR);
end;

procedure TListaNotasDebitoVenta.btnfiltrarClick(Sender: TObject);
begin
  inherited;
    ...
    ZQGrilla.SQL.Text:='select * from documentos '+
                       ...
                       ' where tipodocu_nombre="'+TIPODOCU_NOTADEBITOVENTA+'" '+Princ.empresa_where;
...
procedure TListaNotasDebitoVenta.btnmodificarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_NOTADEBITOVENTA,ABM_MODIFICAR);
end;

procedure TListaNotasDebitoVenta.btnnuevoClick(Sender: TObject);
begin
  inherited;
    Princ.AbrirDocumento('',TIPODOCU_NOTADEBITOVENTA,ABM_AGREGAR);
end;
