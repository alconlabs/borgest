...
type
  TListaFacturasVenta = class(Tlistabase)
...
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_FACTURAVENTA,ABM_ANULAR);
end;

procedure TListaFacturasVenta.btneliminarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_FACTURAVENTA,ABM_ELIMINAR);
end;
...
begin
  inherited;
    ...
    ZQGrilla.SQL.Text:='select * from documentos '+
                       ...
                       ' where tipodocu_nombre="'+TIPODOCU_FACTURAVENTA+'" '+Princ.empresa_where;
...
procedure TListaFacturasVenta.btnmodificarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.RecordCount>0 then
      Princ.AbrirDocumento(ZQGrilla.FieldByName(campo_id).AsString,TIPODOCU_FACTURAVENTA,ABM_MODIFICAR);
end;

procedure TListaFacturasVenta.btnnuevoClick(Sender: TObject);
begin
  inherited;
    Princ.AbrirDocumento('',TIPODOCU_FACTURAVENTA,ABM_AGREGAR);
end;
