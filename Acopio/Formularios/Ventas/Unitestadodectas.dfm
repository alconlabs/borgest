  ...
  object ZQPendientes: TZQuery
    ...
    SQL.Strings = (
      'select *, '
        ...
        'ento_total,0)) as debito, '
      
        'sum(if(tiposdocumento.tipodocu_debcred="CREDITO",documentos.docu' +
        'mento_total,0)) as credito, '
      '0.00 as acumulado, (documento_id) as grupo,'
      
        ' if(documento_fecha<"2012-11-19","Saldo anterior",tiposdocumento' +
        '.tipodocu_nombre) as documentonombre, '
      
        ' if(documento_fecha<"2012-11-19","2012-11-19",documento_fecha) a' +
        's documentofecha, '
      
        ' if(documento_fecha<"2012-11-19","0",puntoventa_numero) as punto' +
        'ventanumero, '
      
        ' if(documento_fecha<"2012-11-19","0",documento_numero) as docume' +
        'ntonumero '
      ' from documentos '
      
        ' inner join tiposdocumento on documentos.tipodocu_id=tiposdocume' +
        'nto.tipodocu_id '
      
        ' inner join puntodeventa on tiposdocumento.puntoventa_id=puntode' +
        'venta.puntoventa_id '
      
        ' inner join entidades on documentos.entidad_id=entidades.entidad' +
        '_id')
    Params = <>
    Left = 272
    Top = 288
