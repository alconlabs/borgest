...
type
  Tlistaordendecarga = class(Tlistabase)
...
var
  listaordendecarga: Tlistaordendecarga;

implementation

uses UnitPrinc, UnitOrdenDeCarga;

{$R *.dfm}

procedure Tlistaordendecarga.btnfiltrarClick(Sender: TObject);
begin
  inherited;
...
procedure Tlistaordendecarga.btnmodificarClick(Sender: TObject);
begin
  inherited;
    if ZQGrilla.Active then
      begin
          if ZQGrilla.RecordCount>0 then
            begin
                try
                  OrdendeCarga:=TOrdendeCarga.Create(self);
                finally
                  OrdendeCarga.abm:=2;
                  OrdendeCarga.tipodocu_nombre:='Orden de Carga';
                  OrdendeCarga.id:=self.id;
                  OrdendeCarga.btnguardar.Caption:='Guardar';
                  OrdendeCarga.Show;
                end;
            end;
      end;
...
procedure Tlistaordendecarga.btnnuevoClick(Sender: TObject);
begin
  inherited;
    try
      OrdendeCarga:=TOrdendeCarga.Create(self);
